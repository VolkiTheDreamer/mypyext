...
def getSepecificColumns_(self,idx,incl_or_excl="excl"):
    """
        Extension method for numpy array type. Returns specified columns from 2D array.
        First, forbiddenfruit must be installed via https://pypi.org/project/forbiddenfruit/
        x is array/list
    """
    if incl_or_excl=="excl":
        idx=np.delete(np.arange(self.shape[1]),idx)
    return self[:,idx]
...
def argwhere_(seri,value):
    """
    Returns the index of the value.
    First, pandas_flavor must be installed via https://pypi.org/project/pandas_flavor/  
    """    
    return seri[seri==value].index[0]

@register_series_method
def itemsContainingLike_(seri,value):
    """
    Returns the index of the value.
    First, pandas_flavor must be installed via https://pypi.org/project/pandas_flavor/  
    """    
    return [x for x in seri if value in str(x)]
