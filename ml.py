...
def plot_precision_recall_curve(y_test_encoded,X_test,estimator,threshs=np.linspace(0.0, 0.98, 40),figsize=(16,6)):
    ...
    for thresh in threshs:
        with warnings.catch_warnings():
            ...
            try:
                ...
                Yr.append(recall_score(y_test_encoded, y_pred,zero_division=1))
            except Warning as e:
    ...
    plt.plot([1, 1], [1, no_skill], color='blue')
    plt.title('PR Curve')
...
def plot_gain_and_lift(estimator, X_test, y_test, pos_label="Yes", figsize=(16, 6)):
    ...
    plt.plot(lg["Decile"], [10 * (i + 1) for i in range(len(lg))], label="Random")
...
def plot_gain_and_lift_decile_Error(estimator,X_test,y_test,pos_label="Yes",figsize=(16,6)):
    """    
        y_test as numpy array
        prints the gain and lift values and plots the charts.    
    """
    ...
    if len(prob_df['Prob'].unique()) < 10:
        num_quantiles = min(10, len(prob_df['Prob'].unique()))
        prob_df['Decile'] = pd.qcut(prob_df['Prob'], num_quantiles, labels=False, duplicates='drop')
    else:
        prob_df['Decile'] = pd.qcut(prob_df['Prob'], 10, labels=False)
