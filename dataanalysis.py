    ...
    try:
        ...
        for k,v in dict_.items():
            if int(v)<=i:
                ...
                print(df[k].value_counts(dropna=False),end="\n\n")
...
def findNullLikeValues(df,listofvalues=[[-1,-999],["-","na","yok","tanımsız","bilinmiyor","?"]]): 
    ...
    for f in df.select_dtypes("number").columns:
        ...
        if x>0:    
            print(f"{x} null-like values,which are '{','.join(set([str(x) for x in values_n]))}', in {f}")
...
def plotTargetByCats(df, cats, target, subplot_tpl, shrink=0.9,bins=10):
    warnings.warn("Warning...plotTargetByCats is deprecated. Use plotTargetByCatsWithN instead")

def plotTargetByCatsWithN(
    df,
    cats,
    target,
    subplot_tpl=(2, 4),
    mode="stacked_percent",     # "stacked_percent" | "yes_rate_ci"
    pos_label="Yes",            # target positive class label
    shrink=0.9,
    bins=10,
    show_n=True,                # show n per category
    ci=0.95,                    # only used in yes_rate_ci
    figsize=None
):
    """
    args:
        cats: categorical features list
        target: target column name
        subplot_tpl: layout of the subplot (r*c)
        mode:
          - "stacked_percent": stacked percent bars (your original style)
          - "yes_rate_ci": P(target=pos_label) with Wilson CI error bars
    """
    ...
    r, c = subplot_tpl
    features = [x for x in cats if x != target]
    ...
    fig, axes = plt.subplots(r, c, figsize=fs, constrained_layout=True)
    axes = np.array(axes).reshape(-1)

    z = 1.96 if ci == 0.95 else 1.96  # keep simple; can be extended with scipy if desired

    for e, cat in enumerate(features):
        if e >= len(axes):
            break
        ax = axes[e]

        if mode == "stacked_percent":
            # Build percent table
            tmp = (
            ...
            # Optional: annotate n on top of each stacked bar
            if show_n:
                n_per_cat = df.groupby(cat).size()
                # Get the x tick positions (0..k-1) and align with category order used in plot
                # seaborn/mpl uses sorted unique categories as ticks in most cases
                cats_order = [t.get_text() for t in ax.get_xticklabels()]
                for i, lab in enumerate(cats_order):
                    if lab in n_per_cat.index.astype(str):
                        n_val = int(n_per_cat.loc[n_per_cat.index.astype(str) == lab].values[0])
                        ax.text(i, 102, f"n={n_val}", ha="center", va="bottom", fontsize=9)

        elif mode == "yes_rate_ci":
            # Compute p_yes and Wilson CI per category
            g = df.groupby(cat)[target].agg(
                n="count",
                k=lambda s: (s == pos_label).sum()
            ).reset_index()

            g_poz= df[df[target]==pos_label].groupby(cat)[target].agg(
                n="count",
                k=lambda s: (s == pos_label).sum()
            ).reset_index()

            g["p"] = g["k"] / g["n"]
            ci_bounds = g.apply(lambda row: _wilson_ci(row["k"], row["n"], z=z), axis=1)
            g["low"] = [b[0] for b in ci_bounds]
            g["high"] = [b[1] for b in ci_bounds]

            # Bar plot for p
            sns.barplot(data=g, x=cat, y="p", ax=ax)

            # Error bars
            x = np.arange(len(g))
            y = g["p"].values
            yerr = np.vstack([(y - g["low"].values), (g["high"].values - y)])
            ...
            # annotate n on bars
            if show_n:
                # for i, n_val in enumerate(g["n"].values):
                #     ax.text(i, y[i] + 0.03, f"n={int(n_val)}", ha="center", fontsize=9)
                for i, n_val in enumerate(g["n"].values):
                    bar_height = y[i]

                    # bar yeterince yüksekse içine yaz
                    if bar_height >= 0.2:
                        ax.text(
                            i,
                            bar_height / 2,
                    ...
                    # bar çok kısa ise üstüne yaz (fallback),
                    else:
                        ax.text(
                            i,
                            bar_height + 0.12,
            ...
            raise ValueError('mode must be "stacked_percent" or "yes_rate_ci"')
...
def plot_binned_logodds_all(
    df,
    numeric_cols,
    target,
    pos_label="Yes",
    q=10,                 # bin sayısı (quantile bins)
    smooth_eps=0.5,       # p=0/1 olmasın diye smoothing
    cols=4,               # subplot sütun sayısı
    figsize=(18, 10),
    add_trendline=True    # basit lineer trend çizgisi
):
    def _to_binary_target(y, pos_label="Yes"):
        """Target'ı 0/1'e çevirir (Yes/No veya 0/1)."""
        if y.dtype == "O":
            return (y == pos_label).astype(int)
        # numeric ise zaten 0/1 varsay
        return y.astype(int)

    y = _to_binary_target(df[target], pos_label=pos_label)
    ...
    for i, col in enumerate(numeric_cols):
        ...
        # quantile binning (her binde benzer örnek sayısı)
        # duplicates="drop" çok az unique değer varsa bin sayısını düşürür
        tmp["bin"] = pd.qcut(tmp["x"], q=q, duplicates="drop")

        g = tmp.groupby("bin").agg(
            n=("y", "size"),
            k=("y", "sum"),
            x_mid=("x", "median")  # bin temsilcisi
        ).reset_index()

        # smoothing: p = (k+eps)/(n+2eps)  -> logit için güvenli
        p = (g["k"] + smooth_eps) / (g["n"] + 2 * smooth_eps)
        logit = np.log(p / (1 - p))

        ax.plot(g["x_mid"], logit, marker="o")
        ax.set_title(col)
        ax.set_xlabel(col)
        ax.set_ylabel("log-odds (binned)")

        # (opsiyonel) basit doğrusal trend
        if add_trendline and len(g) >= 2:
            m, b = np.polyfit(g["x_mid"].values, logit.values, 1)
            xs = np.linspace(g["x_mid"].min(), g["x_mid"].max(), 50)
            ax.plot(xs, m * xs + b);

        # her noktaya bin içi n yaz (istersen kapatabilirsin)
        for xm, lo, nn in zip(g["x_mid"], logit, g["n"]):
            ax.text(xm, lo, f" n={int(nn)}", fontsize=8, va="bottom")

    # boş kalan subplot'ları sil
